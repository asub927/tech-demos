...
- **PR validation**: Every PR that ships or changes a runnable demo must include **at least one screenshot** and **at least one video** of the running app (UI and/or documented CLI + UI flow). **Commit both files in the repo** under `apps/<slug>/docs/validation/` (or a similar in-app path) and link those **relative paths** in the PR body and app README. Cursor/cloud-agent artifact URLs alone are **not** acceptable — they 404 for other viewers.
